...
const Home = () => {
  ...
  return (
    <>
      ...
      {/* Winter Care Tips */}
      <section id="WinterCare" className="py-20 bg-white">
        <div className="max-w-7xl mx-auto px-4">
          <div className="flex items-center gap-3 mb-10" data-aos="fade-right">
            <div className="text-4xl text-purple-600">❄️</div>
            <h2 className="text-3xl md:text-4xl font-bold text-gray-800">Winter Care Tips</h2>
          </div>

          <div className="grid md:grid-cols-2 gap-6">
            {[
              "Keep Them Warm: Provide heated beds and blankets away from drafts.",
              "Limit Outdoor Time: Reduce exposure to cold and wet conditions.",
              "Boost Nutrition: Increase calorie intake for warmth and energy.",
              "Safe Heating: Never let cats sleep directly on heaters.",
              "Groom Regularly: Brushing helps maintain insulating fur coat.",
            ].map((tip, i) => (
              <div
                key={i}
                className="collapse collapse-arrow bg-gradient-to-r from-purple-50 to-pink-50 border border-purple-200 rounded-2xl shadow-md hover:shadow-xl transition-shadow"
                data-aos="fade-up"
                data-aos-delay={i * 100}
              >
                <input type="radio" name="winter-tips" />
                <div className="collapse-title text-lg font-semibold text-purple-800">
                  {tip.split(":")[0]}
                </div>
                <div className="collapse-content">
                  <p className="text-gray-700">{tip.split(":")[1]}</p>
                </div>
              </div>
            ))}
          </div>
